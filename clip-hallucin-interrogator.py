...
def parse_arguments():
    ...
    parser.add_argument('--m_caption', choices=['blip-base', 'blip-large', 'blip2-2.7b', 'blip2-flan-t5-xl', 'git-large-coco'], default='blip-large', type=str, help="The Caption Model you want to use")
    ...
    parser.add_argument('--trippywords', action='store_true', help="Use CLIP hallucinwords; useful for pixelart, psychedelic, abstract things. Not ideal for realistic images / photos.")
...
def create_tokenizer(clip_model_name: str, max_length: int = max_tokens):
    """
    Creates a tokenizer function that replicates open_clip's behavior using the CLIPTokenizer.
    """
...
class Interrogator():
    ...
    def load_clip_model(self):
        ...
        self.flavors = LabelTable(load_list(config.data_path, 'flavors.txt'), "flavors", self)
        self.negative = LabelTable(load_list(config.data_path, 'wCLIPy_negative.txt'), "negative", self)
        self.wCLIPsocialmedia = LabelTable(load_list(config.data_path, 'wCLIPsocialmedia.txt'), "wCLIPsocialmedia", self)
        self.wCLIPobjthings = LabelTable(load_list(config.data_path, 'wCLIPobjthings.txt'), "wCLIPobjthings", self)
        self.wCLIPverbadj = LabelTable(load_list(config.data_path, 'wCLIPverbadj.txt'), "wCLIPverbadj", self)
        self.wCLIPpeople = LabelTable(load_list(config.data_path, 'wCLIPpeople.txt'), "wCLIPpeople", self)
        self.wCLIPcelebs = LabelTable(load_list(config.data_path, 'wCLIPcelebs.txt'), "wCLIPcelebs", self)
        self.wCLIPanimals = LabelTable(load_list(config.data_path, 'wCLIPanimals.txt'), "wCLIPanimals", self)
        self.wCLIPplaces = LabelTable(load_list(config.data_path, 'wCLIPplaces.txt'), "wCLIPplaces", self)
        self.wCLIPUSA = LabelTable(load_list(config.data_path, 'wCLIPUSA.txt'), "wCLIPUSA", self)        
        if args.trippywords:
            self.wCLIPx_bofh = LabelTable(load_list(config.data_path, 'wCLIPx_bofh.txt'), "wCLIPx_bofh", self)
            self.wCLIPz_hallucinwords = LabelTable(load_list(config.data_path, 'wCLIPz_hallucinwords.txt'), "wCLIPz_hallucinwords", self)
        if args.ownwords:
    ...
    def interrogate_classic(self, image: Image, max_flavors: int=max_flavors, caption: Optional[str]=None) -> str:
        ...
        max_sm=max_flavors//4
        max_things=max_flavors//2
      
        wCLIPsocialmedia = ", ".join(self.wCLIPsocialmedia.rank(image_features, max_sm))
        wCLIPobjthings = ", ".join(self.wCLIPobjthings.rank(image_features, max_things))
        wCLIPverbadj = ", ".join(self.wCLIPverbadj.rank(image_features, max_things))
        wCLIPpeople = ", ".join(self.wCLIPpeople.rank(image_features, max_sm))
        wCLIPcelebs = ", ".join(self.wCLIPcelebs.rank(image_features, max_sm))
        wCLIPanimals = ", ".join(self.wCLIPanimals.rank(image_features, max_things))
        wCLIPplaces = ", ".join(self.wCLIPplaces.rank(image_features, max_things))
        wCLIPUSA = ", ".join(self.wCLIPUSA.rank(image_features, max_things))
        artist = self.artists.rank(image_features, 1)[0]
        trending = self.trendings.rank(image_features, 1)[0]
        movement = self.movements.rank(image_features, 1)[0]
        if args.trippywords:
            wCLIPx_bofh = self.wCLIPx_bofh.rank(image_features, 5)[0]
            wCLIPz_hallucinwords = self.wCLIPz_hallucinwords.rank(image_features, 5)[0]
        if args.ownwords:
            ownwords = ", ".join(self.ownwords.rank(image_features, max_things))
        flaves = ", ".join(self.flavors.rank(image_features, max_things))
        if args.debug:
            print(f"def interrogate_classic, Ranked labels: {len(flaves)}, Top-ranked: {flaves[:5]}")

        if args.ownwords and args.trippywords:
            prompt = f"{caption} {artist}, {trending}, {movement}, {ownwords}, {wCLIPsocialmedia}, {wCLIPverbadj}, {wCLIPobjthings}, {wCLIPpeople}, {wCLIPcelebs}, {wCLIPanimals}, {wCLIPplaces}, {wCLIPUSA}, {wCLIPx_bofh}, {wCLIPz_hallucinwords}, {flaves}"  
        elif args.trippywords:
            prompt = f"{caption} {artist}, {trending}, {movement}, {wCLIPsocialmedia}, {wCLIPverbadj}, {wCLIPobjthings}, {wCLIPpeople}, {wCLIPcelebs}, {wCLIPanimals}, {wCLIPplaces}, {wCLIPUSA}, {wCLIPx_bofh}, {wCLIPz_hallucinwords}, {flaves}"
        elif args.ownwords:
            prompt = f"{caption} {artist}, {trending}, {movement}, {ownwords}, {wCLIPsocialmedia}, {wCLIPverbadj}, {wCLIPobjthings}, {wCLIPpeople}, {wCLIPcelebs}, {wCLIPanimals}, {wCLIPplaces}, {wCLIPUSA}, {flaves}"    
        else:
            prompt = f"{caption} {artist}, {trending}, {movement}, {wCLIPsocialmedia}, {wCLIPverbadj}, {wCLIPobjthings}, {wCLIPpeople}, {wCLIPcelebs}, {wCLIPanimals}, {wCLIPplaces}, {wCLIPUSA}, {flaves}"
    ...
    def interrogate_fast(self, image: Image, max_flavors: int=max_flavors, caption: Optional[str]=None) -> str:
        ...
        if args.ownwords and args.trippywords:
            merged = _merge_tables([self.flavors, self.artists, self.trendings, self.movements, self.ownwords, self.wCLIPsocialmedia, self.wCLIPverbadj, self.wCLIPobjthings, self.wCLIPpeople, self.wCLIPcelebs, self.wCLIPanimals, self.wCLIPplaces, self.wCLIPUSA, self.wCLIPx_bofh, self.wCLIPz_hallucinwords], self)
        elif args.trippywords:
            merged = _merge_tables([self.flavors, self.artists, self.trendings, self.movements, self.wCLIPsocialmedia, self.wCLIPverbadj, self.wCLIPobjthings, self.wCLIPpeople, self.wCLIPcelebs, self.wCLIPanimals, self.wCLIPplaces, self.wCLIPUSA, self.wCLIPx_bofh, self.wCLIPz_hallucinwords], self)
        elif args.ownwords:
            merged = _merge_tables([self.flavors, self.artists, self.trendings, self.movements, self.ownwords, self.wCLIPsocialmedia, self.wCLIPverbadj, self.wCLIPobjthings, self.wCLIPpeople, self.wCLIPcelebs, self.wCLIPanimals, self.wCLIPplaces, self.wCLIPUSA], self)
        else:
            merged = _merge_tables([self.flavors, self.artists, self.trendings, self.movements, self.wCLIPsocialmedia, self.wCLIPverbadj, self.wCLIPobjthings, self.wCLIPpeople, self.wCLIPcelebs, self.wCLIPanimals, self.wCLIPplaces, self.wCLIPUSA], self)
